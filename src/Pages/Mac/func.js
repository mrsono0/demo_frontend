...
const map2 = () => {
  ...
  function Displayarea(polygonPath, name, result) {
    ...
    kakao.maps.event.addListener(polygon, "mouseout", function () {
      polygon.setOptions({ fillColor: "#ff3042" });
    });
  ...
  function Hospital() {
    try {
      const response = axios.get("http://127.0.0.1:8000/hospital/");
      var AniHos = sethospital(response.data);

      console.log(AniHos_data);
    } catch (e) {
      console.log(e);
    }
  }
  ...
  function Park() {
    try {
      const response = axios.get("http://127.0.0.1:8000/park/");
